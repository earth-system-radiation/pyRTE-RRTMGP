...
@pytest.mark.parametrize("parameters,method", [
    ((0, 0, 0, np.ones((3, 3, 3)), np.ones((3, 3, 3))),
     py.rte_increment_1scalar_by_1scalar)
])

def test_rte_increment_1scalar_by_1scalar(parameters, method):
    with pytest.raises(RuntimeError) as excinfo:
        method(*parameters)
...
@pytest.mark.parametrize("parameters, method", [
    ((3, 4, 5, np.ones((3, 4, 5)), np.ones((3, 4, 5)), np.ones((3, 4, 5))), py.rte_increment_1scalar_by_2stream)
])
def test_rte_increment_1scalar_by_2stream(parameters, method):
    ...
    for igpt in range(ngpt):
        for ilay in range(nlay):
            for icol in range(ncol):
                res[icol, ilay, igpt] += tau_in[icol, ilay, igpt] * (1.0 - ssa_in[icol, ilay, igpt])
...
def test_rte_increment_1scalar_by_nstream(parameters, method):
    with pytest.raises(RuntimeError) as excinfo:
        method(*parameters)
    assert str(excinfo.value) == "ncol, nlay, and ngpt must be positive integers"
...
def test_rte_increment_1scalar_by_nstream():
    ...
    for igpt in range(ngpt):
        for ilay in range(nlay):
            for icol in range(ncol):
                res[icol - 1, ilay - 1, igpt - 1] += ssa_in[icol - 1, ilay - 1, igpt - 1] * (1.0 - tau_in[icol - 1, ilay - 1, igpt - 1])
...
def test_rte_increment_2stream_by_1scalar(parameters, method):
    with pytest.raises(RuntimeError) as excinfo:
        method(*parameters)
    assert str(excinfo.value) == "ncol, nlay, and ngpt must be positive integers"
    ...
    for igpt in range(1, ngpt + 1):
        for ilay in range(1, nlay + 1):
            for icol in range(1, ncol + 1):
                tau12 = tau_inout[icol - 1, ilay - 1, igpt - 1] + tau_in[icol - 1, ilay - 1, igpt - 1]
                res_ssa_inout[icol - 1, ilay - 1, igpt - 1] = tau_inout[icol - 1, ilay - 1, igpt - 1] * ssa_inout[icol - 1, ilay - 1, igpt - 1] / max(eps, tau12)
                res_tau_inout[icol - 1, ilay - 1, igpt - 1] = tau12

    py.rte_increment_2stream_by_1scalar(*parameters)
    assert np.array_equal(res_tau_inout, tau_inout) and np.array_equal(res_ssa_inout, ssa_inout)
...
@pytest.mark.parametrize("parameters,method", [
    ((0, 0, 0, np.ones((3, 3, 3)), np.ones((3, 3, 3)), np.ones((3, 3, 3)), np.ones((3, 3, 3)), np.ones((3, 3, 3)), np.ones((3, 3, 3))),
     py.rte_increment_2stream_by_2stream)
])

def test_rte_increment_2stream_by_2stream(parameters, method):
    with pytest.raises(RuntimeError) as excinfo:
        method(*parameters)
    assert str(excinfo.value) == "ncol, nlay, and ngpt must be positive integers"

@pytest.mark.parametrize("parameters,method", [
    ((3, 3, 3, np.ones((3, 3)), np.ones((3, 3)), np.ones((3, 3)), np.ones((3, 3)), np.ones((3, 3)), np.ones((3, 3))),
     py.rte_increment_2stream_by_2stream)
])
...
def test_rte_increment_2stream_by_2stream():
    ...
    for igpt in range(1, ngpt + 1):
        for ilay in range(1, nlay + 1):
            for icol in range(1, ncol + 1):
                tau12 = tau_inout[icol - 1, ilay - 1, igpt - 1] + tau_in[icol - 1, ilay - 1, igpt - 1]
                tauscat12 = tau_inout[icol - 1, ilay - 1, igpt - 1] * ssa_inout[icol - 1, ilay - 1, igpt - 1] + tau_in[icol - 1, ilay - 1, igpt - 1] * ssa_in[icol - 1, ilay - 1, igpt - 1]

                res_g_inout[icol - 1, ilay - 1, igpt - 1] = (tau_inout[icol - 1, ilay - 1, igpt - 1] * ssa_inout[icol - 1, ilay - 1, igpt - 1] * g_inout[icol - 1, ilay - 1, igpt - 1] + tau_in[icol - 1, ilay - 1, igpt - 1] * ssa_in[icol - 1, ilay - 1, igpt - 1] * g_in[icol - 1, ilay - 1, igpt - 1]) / max(eps, tauscat12)
                res_ssa_inout[icol - 1, ilay - 1, igpt - 1] = tauscat12 / max(eps, tau12)
                res_tau_inout[icol - 1, ilay - 1, igpt - 1] = tau12

    py.rte_increment_2stream_by_2stream(*parameters)
    assert np.array_equal(res_tau_inout, tau_inout) and np.array_equal(res_ssa_inout, ssa_inout) and np.array_equal(res_g_inout, g_inout)
...
@pytest.mark.parametrize("parameters,method", [
    ((0, 0, 0, 0, np.ones((3, 3, 3)), np.ones((3, 3, 3)), np.ones((3, 3, 3)), np.ones((3, 3, 3)), np.ones((3, 3, 3)), np.ones((3, 3, 3, 3))),
     py.rte_increment_2stream_by_nstream)
])

def test_rte_increment_2stream_by_nstream(parameters, method):
    with pytest.raises(RuntimeError) as excinfo:
        method(*parameters)
    assert str(excinfo.value) == "ncol, nlay, ngpt and nmom2 must be positive integers"

@pytest.mark.parametrize("parameters,method", [
    ((3, 3, 3, 3, np.ones((3, 3)), np.ones((3, 3)), np.ones((3, 3)), np.ones((3, 3)), np.ones((3, 3)), np.ones((3, 3))),
     py.rte_increment_2stream_by_nstream)
])
    ...
    for igpt in range(ngpt):
        for ilay in range(nlay):
            for icol in range(ncol):
                ...
                tauscat12 = (tau_inout[icol, ilay, igpt] * ssa_inout[icol, ilay, igpt] +
                             tau_in[icol, ilay, igpt] * ssa_in[icol, ilay, igpt])
                expected_g_inout[icol, ilay, igpt] = (tau_inout[icol, ilay, igpt] * ssa_inout[icol, ilay, igpt] * g_inout[icol, ilay, igpt] +
                                         tau_in[icol, ilay, igpt] * ssa_in[icol, ilay, igpt] * p_in[icol, ilay, igpt, 0]) \
                                       / max(tauscat12, eps)
                expected_ssa_inout[icol, ilay, igpt] = tauscat12 / max(eps, tau12)
                expected_tau_inout[icol, ilay, igpt] = tau12
                
    py.rte_increment_2stream_by_nstream(ncol, nlay, ngpt, nmom, tau_inout, ssa_inout, g_inout, tau_in, ssa_in, p_in)

    assert np.allclose(expected_tau_inout - tau_inout, 0.)
    assert np.allclose(expected_ssa_inout - ssa_inout, 0.)
    assert np.allclose(expected_g_inout - g_inout, 0.)
...
def test_rte_increment_nstream_by_1scalar(parameters, method):
    with pytest.raises(RuntimeError) as excinfo:
        method(*parameters)
    assert str(excinfo.value) == "ncol, nlay and ngpt must be positive integers"
...
def test_rte_increment_nstream_by_1scalar():
    ...
    for icol in range(ncol):
        for ilay in range(nlay):
            for igpt in range(ngpt):
                tau12 = tau_inout[icol, ilay, igpt] + tau_in[icol, ilay, igpt]
                res_g_inout[icol, ilay, igpt] = tau_inout[icol, ilay, igpt] * g_inout[icol, ilay, igpt] / max(tau12, eps)
...
@pytest.mark.parametrize("parameters,method", [
    ((0, 0, 0, 0, np.ones((3, 3, 3)), np.ones((3, 3, 3)), np.ones((3, 3, 3, 3)), np.ones((3, 3, 3)), np.ones((3, 3, 3)), np.ones((3, 3, 3))),
     py.rte_increment_nstream_by_2stream)
])

def test_rte_increment_nstream_by_2stream(parameters, method):
    with pytest.raises(RuntimeError) as excinfo:
        method(*parameters)
...
def test_rte_increment_nstream_by_2stream():
    ...
    for icol in range(ncol):
        for ilay in range(nlay):
            for igpt in range(ngpt):
                tau12 = tau_inout[icol, ilay, igpt] + tau_in[icol, ilay, igpt]
                tauscat12 = tau_inout[icol, ilay, igpt] * ssa_inout[icol, ilay, igpt] + tau_in[icol, ilay, igpt] * ssa_in[icol, ilay, igpt]

                temp_moms[0] = g_in[icol, ilay, igpt]
                for imom in range(1, nmom1):
                    temp_moms[imom] = temp_moms[imom - 1] * g_in[icol, ilay, igpt]
                
                p_a = tau_inout[icol, ilay, igpt] * ssa_inout[icol, ilay, igpt] * p_inout[icol, ilay, igpt, 0:nmom1]
                p_b = tau_in[icol, ilay, igpt] * ssa_in[icol, ilay, igpt] * temp_moms[0:nmom1]
    ...
    py.rte_increment_nstream_by_2stream(ncol, nlay, ngpt, nmom1, tau_inout, ssa_inout, p_inout, tau_in, ssa_in, g_in)

    assert np.allclose(expected_tau_inout - tau_inout, 0.)
    assert np.allclose(expected_ssa_inout - ssa_inout, 0.)
    assert np.allclose(expected_phase_inout - p_inout, 0.)
...
@pytest.mark.parametrize("parameters,method", [
    ((0, 0, 0, 0, 0, np.ones((3, 3, 3)), np.ones((3, 3, 3)), np.ones((3, 3, 3, 3)), np.ones((3, 3, 3)), np.ones((3, 3, 3)), np.ones((3, 3, 3, 3))),
     py.rte_increment_nstream_by_nstream)
])

def test_rte_increment_nstream_by_nstream(parameters, method):
    with pytest.raises(RuntimeError) as excinfo:
        method(*parameters)
    assert str(excinfo.value) == "ncol, nlay, ngpt, nmom1 and nmom2 must be positive integers"

@pytest.mark.parametrize("parameters,method", [
    ((3, 3, 3, 3, 3, np.ones((3, 3)), np.ones((3, 3)), np.ones((3, 3)), np.ones((3, 3)), np.ones((3, 3)), np.ones((3, 3))),
     py.rte_increment_nstream_by_nstream)
])
...
def test_rte_increment_nstream_by_nstream():
    ...
    py.rte_increment_nstream_by_nstream(ncol, nlay, ngpt, nmom1, nmom2, tau_inout, ssa_inout, p_inout, tau_in, ssa_in, p_in)
    assert np.allclose(expected_tau_inout - tau_inout, 0.)
    assert np.allclose(expected_ssa_inout - ssa_inout, 0.)
    assert np.allclose(expected_phase_inout - p_inout, 0.)