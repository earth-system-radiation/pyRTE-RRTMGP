...
PYBIND11_MODULE(pyrte, m) {
    ...
    m.def("rte_sw_solver_2stream", [](int ncol, int nlay, int ngpt, bool top_at_1,
        ...
        if (tau.size() != ncol * nlay * ngpt || ssa.size() != ncol * nlay * ngpt || g.size() != ncol * nlay * ngpt ||
            mu0.size() != ncol * nlay || sfc_alb_dir.size() != ncol * ngpt || sfc_alb_dif.size() != ncol * ngpt ||
            inc_flux_dir.size() != ncol * ngpt || flux_up.size() != ncol * (nlay + 1) * ngpt ||
            flux_dn.size() != ncol * (nlay + 1) * ngpt || flux_dir.size() != ncol * (nlay + 1) * ngpt ||
            inc_flux_dif.size() != ncol * ngpt || broadband_up.size() != ncol * (nlay + 1) ||
            broadband_dn.size() != ncol * (nlay + 1) || broadband_dir.size() != ncol * (nlay + 1)) {
            throw std::runtime_error("Invalid size for input arrays");
        }

        int top_at_1_int = top_at_1 ? 1 : 0;
        int has_dif_bc_int = has_dif_bc ? 1 : 0;
        int do_broadband_int = do_broadband ? 1 : 0;
        ...
        fortran::rte_sw_solver_2stream(&ncol, &nlay, &ngpt, &top_at_1_int,
                                    static_cast<double *>(buf_tau.ptr), static_cast<double *>(buf_ssa.ptr),
                                    static_cast<double *>(buf_g.ptr), static_cast<double *>(buf_mu0.ptr),
                                    static_cast<double *>(buf_sfc_alb_dir.ptr), static_cast<double *>(buf_sfc_alb_dif.ptr),
                                    static_cast<double *>(buf_inc_flux_dir.ptr), static_cast<double *>(buf_flux_up.ptr),
                                    static_cast<double *>(buf_flux_dn.ptr), static_cast<double *>(buf_flux_dir.ptr),
                                    &has_dif_bc_int, static_cast<double *>(buf_inc_flux_dif.ptr), &do_broadband_int,
                                    static_cast<double *>(buf_broadband_up.ptr), static_cast<double *>(buf_broadband_dn.ptr),
                                    static_cast<double *>(buf_broadband_dir.ptr));
    });

    m.def("rte_lw_solver_2stream", [](int ncol, int nlay, int ngpt, bool top_at_1,
                                    py::array_t<double> tau, py::array_t<double> ssa, py::array_t<double> g,
        ...
        if (tau.size() != ncol * nlay * ngpt || ssa.size() != ncol * nlay * ngpt || g.size() != ncol * nlay * ngpt ||
            lay_source.size() != ncol * nlay * ngpt || lev_source.size() != ncol * (nlay + 1) * ngpt ||
            sfc_emis.size() != ncol * ngpt || sfc_src.size() != ncol * ngpt || inc_flux.size() != ncol * ngpt ||
            flux_up.size() != ncol * (nlay + 1) * ngpt || flux_dn.size() != ncol * (nlay + 1) * ngpt) {
            throw std::runtime_error("Invalid size for input arrays");
        }
        ...
        fortran::rte_lw_solver_2stream(&ncol, &nlay, &ngpt, &top_at_1_int,
                                        static_cast<double *>(buf_tau.ptr), static_cast<double *>(buf_ssa.ptr),
                                        static_cast<double *>(buf_g.ptr), static_cast<double *>(buf_lay_source.ptr),
                                        static_cast<double *>(buf_lev_source.ptr), static_cast<double *>(buf_sfc_emis.ptr),
                                        static_cast<double *>(buf_sfc_src.ptr), static_cast<double *>(buf_inc_flux.ptr),
                                        static_cast<double *>(buf_flux_up.ptr), static_cast<double *>(buf_flux_dn.ptr));
    });

    m.def("rte_increment_1scalar_by_1scalar", [](int ncol, int nlay, int ngpt,
                                                  py::array_t<double> tau_inout, py::array_t<double> tau_in) {

        if (tau_inout.size() != ncol * nlay * ngpt || tau_in.size() != ncol * nlay * ngpt) {
            throw std::runtime_error("Invalid size for input arrays");
        }
        ...
        fortran::rte_increment_1scalar_by_1scalar(&ncol, &nlay, &ngpt, static_cast<double *>(buf_tau_inout.ptr), static_cast<double *>(buf_tau_in.ptr));
    });

    m.def("rte_increment_1scalar_by_2stream", [](int ncol, int nlay, int ngpt,
                                                  py::array_t<double> tau_inout, py::array_t<double> tau_in, py::array_t<double> ssa_in) {

        if (tau_inout.size() != ncol * nlay * ngpt || tau_in.size() != ncol * nlay * ngpt || ssa_in.size() != ncol * nlay * ngpt) {
            throw std::runtime_error("Invalid size for input arrays");
        }
        ...
        fortran::rte_increment_1scalar_by_2stream(&ncol, &nlay, &ngpt, static_cast<double *>(buf_tau_inout.ptr), static_cast<double *>(buf_tau_in.ptr), static_cast<double *>(buf_ssa_in.ptr));
    });
